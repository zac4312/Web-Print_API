...
pub async fn create_vendor(con: &Pool<Postgres>, vendor: Vendor) -> Result<(), sqlx::Error> {
    sqlx::query!(
        "
        INSERT INTO vendors (name, pw_hash, email, bw_rate, clrd_rate, location, availability) 
         VALUES ($1, $2, $3, $4, $5, $6, $7::vacancy);
        ",
        vendor.name, vendor.pw_hash, vendor.email, vendor.bw_rate, vendor.clrd_rate, vendor.location, vendor.availability,
    )       
    .execute(con)
    .await?;
